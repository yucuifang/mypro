...
class AllTest:##定义一个类AllTest
    ...
    def set_case_suite(self):
        ...
        for case in self.caseList:#从caselist元素组中循环取出case
            ...
            case_module = case.split("/")[0]
            caseFile = os.path.join(self.caseFile, case_module)#真正的测试断言文件路径
            print(case_name+".py")#打印出取出来的名称
            #批量加载用例，第一个参数为用例存放路径，第一个参数为路径文件名
            discover = unittest.defaultTestLoader.discover(caseFile, pattern=case_name + '.py', top_level_dir=caseFile)
